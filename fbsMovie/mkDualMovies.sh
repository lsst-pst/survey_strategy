...
echo "python example_movie.py "$opsimRun".db --movieStepsize "$stepsize" --nside "$nside" --ips 5 \\
  --sqlConstraint '"$sqlconstraint"' --outDir "$opsimRun"_"$metadata"_cumulative"

echo "python example_movie.py "$opsimRun".db --movieStepsize "$stepsize" --nside "$nside" --ips 5 \\
  --sqlConstraint '"$sqlconstraint"' --binned --outDir "$opsimRun"_"$metadata"_binned"
...
echo "ffmpeg -i "$opsimRun"_"$metadata"_cumulative/"$moviename" \\
 -i "$opsimRun"_"$metadata"_binned/"$moviename" \\
