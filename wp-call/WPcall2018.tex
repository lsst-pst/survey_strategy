...
The git repository (\href{https://github.com/lsst-pst/survey_strategy/}{https://github.com/lsst-pst/survey\_strategy/}) hosting the source of this document also holds additional information relevant to the call for white papers. This includes:
\begin{itemize}
\item The baseline2018a simulated survey document, which describes the recent, post-COSEP, features enabled in OpSim, as well 
as characteristics of the updated baseline simulated survey ('baseline2018a'). This document is available at \url{http://ls.st/3q5}, under 
the baseline2018a-doc directory\footnote{Note, to download PDFs from github, simply click the "Download" button on the webpage that opens 
after clicking the link to the PDF.}
\item The `next candidate reference run' ({\it e.g.} the run that is likely to become baseline2018b) simulated survey document, which describes
some additional updates in OpSim to increase simulation fidelity. These updates include simulating dome crawl, which increases the efficiency 
...
\url{http://ls.st/cm9}, under the baseline2018b-doc directory.
\item A summary of the alternate survey strategy simulations produced as examples for the call for white papers, listed below in Section~\ref{sec:surveys}.
This document, including comparisons of a subset of metric analysis results, is available at \url{http://ls.st/ru8}, under the WPruns directory. 
Additional plots and metric comparisons are available in this same directory, at \url{http://ls.st/blm}.
\end{itemize}

The full outputs of MAF analyses for the 2018a baseline survey, as well as the example runs demonstrating potential options for mini surveys (a subset of the runs described in Chapter 2 of the COSEP) are available \href{http://astro-lsst-01.astro.washington.edu:8080}{online} at \url{http://astro-lsst-01.astro.washington.edu:8080}.  
...
at the LSST 2018 Project and Community Workshop in Tucson, AZ (August 13-17, 2018). Another highly relevant
...
application form at http://ls.st/cwg by July 20, 2018. Finally, the Project will organize a dedicated session at the
...
\item A few examples of ``rolling cadence'' simulations of the main survey -- concentrating observations in bands of declination over varying numbers of years.
\end{itemize}
