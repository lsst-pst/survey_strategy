...
While the footprint of the main LSST survey is fairly well-defined (Declination -62 to +2 degrees, excluding
the center of the galactic plane), the cadence of observations within
...
additional surveys are not well defined. We are soliciting white papers to help plan the LSST 
survey strategy in these areas. 

The anticipated due date for these white papers in November 20, 2018.  {\it do we have a proper date? need to avoid holidays}
}
...
10-year observing strategy can deliver on a wide range of science goals and meet the core requirements
as described in the LSST Science Requirements Document (\href{SRD}{http://ls.st/srd}).
The \href{LSST Overview paper}{http://ls.st/pif}\footnote{A living document available at \href{http://ls.st/pif}{http://ls.st/pif}.}
provides a concise background of the primary LSST science goals of
\begin{itemize}
...
\end{itemize}
and how they drive the basic requirements of the LSST survey strategy and data processing, as well as a basic description
of the baseline survey strategy itself.
...
is where this work is being assembled, in the form of a large cross-community survey evaluation paper
titled \href{`Science-Driven Optimization of the LSST Survey Strategy'}{http://ls.st/9fw}\footnote{The first 
...
Through the end of construction and commissioning, this community observing strategy evaluation paper will 
...
The LSST \href{SRD}{http://ls.st/srd}\footnote{See ls.st/srd} 
...
but this is not a formal optimization effort in part because the concept of an ``optimized'' survey is difficult to define. Another way
of looking at this effort is that we are attempting to ``minimize the unhappiness of the broad LSST community with the survey strategy''.}
